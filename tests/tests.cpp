...
int main(){
 ...
 { JobScheduler s(1);s.add(make("a",Priority::High,{},[]{return true;}));s.start();s.wait();bool tagged=false;for(const auto& event:s.events())if(event.status==Status::Running&&event.worker_id==1)tagged=true;assert(tagged); }
